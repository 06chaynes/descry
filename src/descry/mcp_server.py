...
mcp = FastMCP("descry", lifespan=server_lifespan)


def _svc() -> DescryService:
    assert _service is not None, "Server not initialized"
    return _service
...
async def descry_callers(name: str, limit: int = 20) -> str:
    ...
    return await _svc().callers(name, limit)
...
async def descry_callees(name: str, limit: int = 20) -> str:
    ...
    return await _svc().callees(name, limit)
...
async def descry_flow(
    ...
    return await _svc().flow(start, direction.value, depth, target, inline_threshold)
...
async def descry_structure(filename: str) -> str:
    ...
    return await _svc().structure(filename)
...
async def descry_flatten(class_node_id: str) -> str:
    ...
    return await _svc().flatten(class_node_id)
...
async def descry_semantic(query: str, limit: int = 10) -> str:
    ...
    return await _svc().semantic(query, limit)
...
async def descry_quick(name: str, full: bool = False, brief: bool = False) -> str:
    ...
    return await _svc().quick(name, full, brief)
...
async def descry_impls(method: str, trait_name: str | None = None) -> str:
    ...
    return await _svc().impls(method, trait_name)
...
async def descry_path(
    ...
    return await _svc().path(start, end, max_depth, direction.value)
...
async def descry_evolution(
    ...
    return await _svc().evolution(name, time_range, limit, show_diff, crate)
